            ...
            if (event.length < 1) res.status(500).send("No element in req.body.event")
                ...
                async.each(event, function(element, next) {
                    ...
                    if (action === "put") {
                        ...
                        console.log("put")
                        thisOffer = new Offer({
                            title: serviceProvider.length + "/" + serviceProviderNumberLimit,
...
router.post('/service', limiterPost.middleware({
    ...
    if (!req.body.creator || !req.body.service) {
        return res.status(500)
            .send("No serviceName or userId or password or serviceProvider_id or category or imageURL ")
    } else if (!req.body.service.serviceName || !req.body.service || !req.body.creator.password || !req.body.creator.id || !req.body.creator._id || !req.body.service.category || !req.body.startTime || !req.body.endTime) {
        console.log(req.body)
        return res.status(500)
